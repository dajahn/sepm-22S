...
@DataJpaTest
@ActiveProfiles("test")
public class OrderRepositoryTest implements LocationTestData, EventTestData, ArtistTestData, UserTestData, AddressTestData {
    ...
    public void givenNothing_whenSaveEventAndPurchaseOrder_thenFindPurchasedUpcomingOrPastTickets() {
        ...
        User finalUser = user;
        TicketOrder finalOrder = order;
        assertAll(
            () -> assertEquals(finalOrder, orderRepository.findTicketOrdersByTypeAndUserId(OrderType.PURCHASE, finalUser.getId()).get(0)),
            () -> assertEquals(1, orderRepository.findTicketOrdersByTypeAndUserId(OrderType.PURCHASE, finalUser.getId()).size())
        );
    }
}
