...
public interface SeatTicketRepository extends JpaRepository<SeatTicket, Long> {
    ...
    @Query("select (count(s) > 0) from SeatTicket s where s.cancellation is null and s.performance.id = ?1 and s.seat.id = ?2 and (s.order.validUntil is null or s.order.validUntil > current_date)")
