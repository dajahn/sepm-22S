...
public interface StandingTicketRepository extends JpaRepository<StandingTicket, Long> {
    ...
    @Query("select count(s) from StandingTicket s where s.performance.id = ?1 and s.sector.id = ?2 and (s.order.validUntil is null or s.order.validUntil > current_date)")
